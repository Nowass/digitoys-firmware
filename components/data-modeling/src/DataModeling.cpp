...
namespace digitoys::datamodeling
{
    ...
    BehaviorDataPoint DataModeling::processRawData(const std::vector<digitoys::datalogger::DataEntry> &raw_data)
    {
        ...
        // Store the data point in ring buffer
        pushDataPoint(data_point);
        total_data_points_++;
    ...
    {
        std::vector<BehaviorDataPoint> snapshot;
        ...
        {
            if (dp.session_data.session_id == target_session)
            {
    ...
    std::string DataModeling::exportToCSV(const ExportConfig &config) const
    {
        ...
        std::vector<BehaviorDataPoint> snapshot;
        snapshotBuffer(snapshot);
        for (const auto &data_point : snapshot)
        {
    ...
    void DataModeling::snapshotBuffer(std::vector<BehaviorDataPoint> &out) const
    {
        out.clear();
        // Fast path: if empty, return
        if (size_ == 0)
            return;
        ...
        size_t local_head = head_;
        if (buffer_mutex_)
            xSemaphoreGive(buffer_mutex_);

        out.reserve(local_size);
        // Compute index of oldest element
        size_t oldest_idx = (local_head + kBufferCapacity - local_size) % kBufferCapacity;
        for (size_t i = 0; i < local_size; ++i)
        {
            size_t idx = (oldest_idx + i) % kBufferCapacity;
            out.push_back(buffer_[idx]);
        }
    }

    size_t DataModeling::getLatestN(size_t n, std::vector<BehaviorDataPoint> &out) const
    {
        out.clear();
        if (n == 0 || size_ == 0)
            return 0;
        ...
        size_t count = (n < local_size) ? n : local_size;
        out.reserve(count);
        // Newest item is at (head_ - 1 + cap) % cap
        for (size_t i = 0; i < count; ++i)
        {
            size_t idx = (local_head + kBufferCapacity - 1 - i) % kBufferCapacity;
            out.push_back(buffer_[idx]);
        }
